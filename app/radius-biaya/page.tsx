...
import { getAllSkRadiusBiaya, deleteSkRadiusBiaya, type SkRadiusBiaya } from '@/lib/api';
...
export default function SkRadiusBiayaList() {
    const [data, setData] = useState<SkRadiusBiaya[]>([]);
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const result = await getAllSkRadiusBiaya();
    ...
    const handleDelete = async () => {
        if (!deleteId) return;
        try {
            const result = await deleteSkRadiusBiaya(deleteId);
            if (result.success) {
        ...
        <div className="space-y-6">
            <BlurFade delay={0.1} inView>
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight">Radius Biaya Perkara</h2>
                        <p className="text-muted-foreground">Surat Keputusan Panjar Biaya Perkara Pengadilan Agama Penajam.</p>
                    </div>
                    <Link href="/radius-biaya/tambah">
                ...
                <Card>
                    <CardHeader className="space-y-4 pb-4">
                        <div className="flex flex-row items-center justify-between">
                            <CardTitle className="text-lg font-medium">Daftar SK Radius Biaya</CardTitle>
                                    ...
                                    {loading ? (
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={7} className="h-24 text-center">
                                                Belum ada SK Radius Biaya. Klik &quot;Tambah SK&quot; untuk memulai.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        data.map((item, index) => (
                                            <TableRow key={item.id}>
                                                ...
                                                <TableCell className="font-medium text-emerald-700 max-w-[250px] truncate" title={item.nomor_sk}>
                                                    {item.nomor_sk}
                                                </TableCell>
                                                ...
                                                <TableCell className="text-right">
                                                    <div className="flex justify-end gap-2">
                                                        <Link href={`/radius-biaya/${item.id}/edit`}>
